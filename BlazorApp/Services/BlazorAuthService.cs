...
namespace BlazorApp.Services
{
    public class BlazorAuthService : IAuthService
    {
        ...
        public async Task<UserDTO?> GetCurrentUserAsync()
        {
            try
            {
                // Primero intento leer el objeto usuario guardado en localStorage
                var userJson = await _js.InvokeAsync<string>("localStorage.getItem", USER_KEY);
                if (!string.IsNullOrEmpty(userJson))
                {
                    var opts = new JsonSerializerOptions { PropertyNameCaseInsensitive = true };
                ...
                // Si no hay objeto completo, devuelvo un DTO parcial con el username almacenado.
                // Evitamos llamar a la API durante el bootstrap para no depender de la inicialización del auth provider.
                var username = await GetUsernameAsync();
                if (string.IsNullOrEmpty(username)) return null;
